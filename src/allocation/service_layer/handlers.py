...
def reallocate(event: events.Deallocated, uow: unit_of_work.AbstractUnitOfWork) -> None:
    with uow:
...
def add_allocation_to_read_model(event: events.Allocated, uow: unit_of_work.SqlAlchemyUnitOfWork) -> None:
    with uow:
        uow.session.execute(
...
def remove_allocation_from_read_model(event: events.Deallocated, uow: unit_of_work.SqlAlchemyUnitOfWork) -> None:
    with uow:
        uow.session.execute(
